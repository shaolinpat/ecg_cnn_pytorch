...
# explorer/run_streamlit_ecg_app.py
...
This version presents model choices that make sense:
  ...
  • Bundled sample — explorer/checkpoints/sample_model_ECGCovNet.pth
...
# --- Page cleanliness toggles ---
SHOW_PREVIEW_TABLE = False  # df.head()
SHOW_NUMERIC_TABLES = False  # confidences table + SHAP table
...
if candidates:
    ...
    st.info(
        ...
        "• Include `explorer/checkpoints/sample_model_ECGConvNet.pth`"
    )
...
sample_id = Path(uploaded_file.name).stem
...
# Save waveform consistently
waveform_png = images_dir / f"{sample_id}_waveform.png"
fig.savefig(waveform_png, dpi=300)

st.pyplot(fig)
st.write("File received:", uploaded_file.name)
fig.savefig(waveform_png, dpi=300)  # harmless duplicate
st.caption(f"Waveform saved as: {waveform_png.as_posix()}")
...
# Build confidence DataFrame (saved, optional display)
conf_df = pd.DataFrame({"Class": CLASS_NAMES, "Confidence": probs.astype(float)})
if SHOW_NUMERIC_TABLES:
    st.dataframe(conf_df.sort_values("Confidence", ascending=False), hide_index=True)

# Uncomment to get the csvs saved out
# conf_csv = images_dir / f"{sample_id}_confidences.csv"
# conf_df.to_csv(conf_csv, index=False)

st.caption(f"Leads used: {', '.join(used_leads[:12])}")

# --- Confidence bar chart (and export) ---
plot_df = conf_df.sort_values("Class", ascending=True)
...
conf_chart = (
    ...
    .mark_bar()
    .encode(
        x=alt.X("Class:N", sort="ascending", axis=alt.Axis(labelAngle=0)),
        y=alt.Y("Confidence:Q", scale=alt.Scale(domain=[0, 1])),
        tooltip=["Class", alt.Tooltip("Confidence:Q", format=".4f")],
    )
)
...
_saved = False
try:
    conf_chart.save(images_dir / f"{sample_id}_prediction_confidences.svg")
    _saved = True
except Exception:
    try:
        conf_chart.save(
            images_dir / f"{sample_id}_prediction_confidences.png", scale_factor=2.0
        )
        _saved = True
    except Exception:
        pass
...
if not _saved:
    ...
    axc.bar(plot_df["Class"], plot_df["Confidence"])
    axc.set_ylim(0, 1)
    axc.set_xlabel("Class")
    axc.set_ylabel("Confidence")
    axc.set_title("Prediction confidences", pad=20)
    for i, v in enumerate(plot_df["Confidence"]):
        axc.text(i, v + 0.01, f"{v:.4f}", ha="center", va="bottom", fontsize=9)
    figc.tight_layout()
    figc.savefig(images_dir / f"{sample_id}_prediction_confidences.png", dpi=300)
    plt.close(figc)

# --- SHAP per-lead attribution  ---
try:
    ...
    # Optional numeric table (hidden by default)
    if SHOW_NUMERIC_TABLES:
    ...
    chart = (
        ...
        .encode(
            ...
            tooltip=[alt.Tooltip("Lead:N"), alt.Tooltip("Importance:Q", format=".2e")],
        )
    )

    # Export SHAP bar chart for README (SVG preferred, fallback to PNG)
    saved = False
    try:
        # Requires: pip install altair_saver vl-convert-python
        chart.save(images_dir / f"{sample_id}_shap_importance.svg")
        saved = True
    except Exception:
        try:
            chart.save(
                images_dir / f"{sample_id}_shap_importance.png", scale_factor=2.0
            )  # altair PNG path
            saved = True
        except Exception:
            pass
    ...
    if not saved:
        # Fallback: matplotlib export
        fig_shap, ax_shap = plt.subplots()
        ax_shap.bar(imp_df["Lead"], imp_df["Importance"])
        ax_shap.set_title("Per-lead importance (SHAP)")
        ax_shap.set_xlabel("Lead")
        ax_shap.set_ylabel("Importance")
        plt.xticks(rotation=40, ha="center")
        fig_shap.tight_layout()
        fig_shap.savefig(images_dir / f"{sample_id}_shap_importance.png", dpi=300)
        plt.close(fig_shap)
