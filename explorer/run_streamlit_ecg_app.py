...
fig.savefig("images/prediction.png", dpi=300)

st.pyplot(fig)
st.write("File received:", uploaded_file.name)

# Save the waveform as PNG in images/
out_dir = Path("images")
out_dir.mkdir(exist_ok=True)
png_name = Path(uploaded_file.name).stem + "_waveform.png"
fig.savefig(out_dir / png_name, dpi=300)
st.caption(f"Waveform saved as: images/{png_name}")
...
conf_df.to_csv("images/prediction_confidences.csv", index=False)

st.caption(f"Leads used: {', '.join(used_leads[:12])}")

# --- Confidence bar chart (and export) ---
plot_df = conf_df.sort_values("Confidence", ascending=False)
...
conf_chart = (
    ...
    .mark_bar()
    .encode(
        x=alt.X("Class:N", sort=list(plot_df["Class"]), axis=alt.Axis(labelAngle=0)),
        y=alt.Y("Confidence:Q", scale=alt.Scale(domain=[0, 1])),
        tooltip=["Class", alt.Tooltip("Confidence:Q", format=".3f")],
    )
)
...
_saved = False
try:
    conf_chart.save("images/prediction_confidences.svg")  # vector, perfect when zoomed
    _saved = True
except Exception:
    try:
        conf_chart.save("images/prediction_confidences.png", scale_factor=2.0)
        _saved = True
    except Exception:
        pass
...
if not _saved:
    ...
    axc.set_title("Prediction confidences")
    for i, v in enumerate(plot_df["Confidence"]):
        axc.text(i, v + 0.02, f"{v:.2f}", ha="center", va="bottom", fontsize=9)
    figc.tight_layout()
    figc.savefig("images/prediction_confidences.png", dpi=300)
    plt.close(figc)
...
try:
    ...
    chart = (
        ...
        .encode(
            x=alt.X(
                ...
                axis=alt.Axis(
                    labelAngle=40, labelAlign="right", labelBaseline="middle"
                ),
            ),
    ...
    try:
        ...
        chart.save("images/shap_importance.svg")
        saved = True
    except Exception:
        try:
            chart.save(
                "images/shap_importance.png", scale_factor=2.0
            )  # altair PNG path
            saved = True
        except Exception:
            pass
    ...
    if not saved:
        ...
        fig_shap.savefig("images/shap_importance.png", dpi=300)
        plt.close(fig_shap)
